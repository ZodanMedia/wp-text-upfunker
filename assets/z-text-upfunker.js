...
class zTextUpfunkerSingle {
	constructor(el, effect, maxLoops) {
		...
		if (this.maxLoops == 0) this.maxLoops = Infinity;
	...
	init() {
		...
		if (!(this.effect in this.effects)) {
			this.effect = this.effect === 'rand' ? this.randomObjectKey(this.effects) : 'code';
		}
	...
	codeAnimateWords() {
		if (!this.animateWordsBuffer) {
			this.animateWordsBuffer = this.messages[this.message].split('').map(char => ({
		...
		for (let fader of this.animateWordsBuffer) {
			if (fader.c > 0) {
				do_cycles = true;
				fader.c--;
	...
	startCharAnimation() {
		this.loopCharAnimationWords();
	}

	loopCharAnimationWords() {
		if (this.currentLoop >= this.maxLoops) return;
		...
		const showNextWord = () => {
			...
			const chars = this.splitTextToChars(this.messages[messageIndex]);
			let charIndex = 0;
	...
	getWordsFromElement() {
		...
		this.messages = text.split(/[ –,-]/g).map(word => word.trim());
		this.el.innerHTML = '';
	}
	...
	generateRandomString(length) {
		return Array.from({ length }, () => this.codecharacters.charAt(Math.floor(Math.random() * this.codecharacters.length))).join('');
	}
...
// Start funking if there is something to do
