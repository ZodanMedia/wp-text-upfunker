...
getWordsFromElement(){let elFirstContent=this.el.innerHTML.trim();let allElements=Array.from(elFirstContent.split(/[–, -]/g));let allElementsTrimmed=allElements.map(string=>string.trim());let allElementsFiltered=allElementsTrimmed.filter(el=>el.length>0);this.messages=allElementsFiltered;this.el.innerHTML=''}
...
class zTextUpfunker{constructor(options){const selector=options.elem||'';const effect=options.type||'code';const maxLoops=options.cycles||0;const elements=document.querySelectorAll(selector);if(!elements.length){return}
